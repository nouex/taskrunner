...
function Task (fn, args, opts) {
  ...
  // opts:logging = true
  ret.logging = "logging" in opts ? Boolean(opts.logging) : true;
  // opts:logStream = process.stdout
  ret.logStream = "logStream" in opts && opts.logStream.write ? opts.logStream :
                  process.stdout;
...
Task.prototype.moveDep = function (id, index, set) {
  ...
  else ret = this.addDep(actual, index, set);
  // return (new id)
  return ret;
};
...
Task.prototype._addSetEntry = function(set, actual, executable, index, once) {
  ...
  // expects exact args
  if (arguments.length !== 5) throw new Error;// TODO use debug() instead
...
Task.prototype._handleSet = function (set) {
  // NOTE returns a copy of set
  set = tool.deepCpy(set);
  this._orderSet(set);
  set.forEach(function(meta, ind) {
    // execute if needed
    if (meta.executable) {
      // rem, sets are copied during execution/run so we can change the value
...
// NOTE: changes in execute() must most likely be synced with ./AsyncTask.js's
// version of execute()
