  ...
  tool.objForEach(this.sets, function(breakout, set) {
    this._traverseSet(set, function(breakout2, meta) {
      ...
      if (isAsyncTask(actual) && !actual.executed) {
        ...
        breakout2();
        breakout();
      } else if (isAsyncTask(actual) /*unecessary, since the prev 'if' checks for asyc task and if an async task has executed (gotten this far) then it did not have to depend on anything so it _mayExevute here is unecessary for async */ ||isTask(actual)){
